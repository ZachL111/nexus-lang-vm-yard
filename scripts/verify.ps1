...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-nexus-lang-vm-detail.ps1

julia --project=. tests/domain_review.jl

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-nexus-lang-vm-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-nexus-lang-vm-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-nexus_lang_vm_yard-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
